...
export const postShippingAddress = {
  ...
  controller: async (req: Request, res: Response, next: NextFunction) => {
    ...
    await shippingAddressService.insertShippingAddress(userId, name, recipient, contact, postcode, address, address_detail, requirement);
...
export const deleteShippingAddress = {
  validator: [
    isLoggedIn,
    param('id').isNumeric().withMessage('배송지 ID는 숫자여야 해요.'),
    validate
  ],
  controller: async (req: Request, res: Response, next: NextFunction) => {
    const loggedUser = req.user as UserType;
    ...
    const [[address]] = await shippingAddressService.selectUserShippingAddressDetail(id);
