...
const controller = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (voucherIds.length !== trade.amount) return res.status(400).json({ message: `사용할 소유권을 ${trade.amount}개 선택해주세요.` });
  ...
  if (voucher.owner.userId !== trade.userId) return res.status(400).json({ message: '교환글의 작성자가 소유권의 실 소유주가 아니에요.' });
