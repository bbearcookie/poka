...
const controller = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (voucher.state !== 'available' && voucher.voucherId !== trade.voucherId) return res.status(404).json(createResponseMessage('haveVoucherId', '사용하려는 소유권이 이용 가능한 상태가 아니에요.'));
