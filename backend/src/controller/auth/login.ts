...
const validator = [
  body('username').trim().toLowerCase().not().isEmpty().withMessage('아이디가 비어있어요.').bail(),
  body('password').not().isEmpty().withMessage('비밀번호가 비어있어요.').bail(),
  validate,
];
...
const controller = async (req: Request, res: Response, next: NextFunction) => {
  ...
  if (!user) {
    return res.status(409).json(createResponseMessage('username', '가입되지 않은 아이디에요.'));
  }

  const encryptedPassword = encryptText(password, user.salt);
  if (user.password !== encryptedPassword) {
    return res.status(409).json(createResponseMessage('password', '비밀번호가 일치하지 않아요.'));
  }
...
// 로그인
const login = [...validator, controller];
