  ...
  return (
    <Card styles={cardStyles}>
      ...
      <CardListItem title="결제상태">
        <StateLabel state={{ type: "payment", key: shipping.payment.state}} styles={{ width: "6em", margin: "0 0.5em 0.2em 0" }} />
        {shipping.payment.state === 'waiting' && <p className="description">아직 배송비를 결제하지 않은 상태입니다.</p>}
        {shipping.payment.state === 'paid' && <p className="description">배송비를 정상적으로 결제한 상태입니다.</p>}
        {shipping.payment.state === 'forgeried' && <p className="description">결제가 위조되었거나 문제가 발생한 상태입니다.</p>}
      </CardListItem>
